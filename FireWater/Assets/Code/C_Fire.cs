...
public class C_Fire : C_Player
{
    void Update()
    {
        if (!C_AlwaysThere.MovementStop)
        {
            ...
            if (Input.GetKey(KeyCode.LeftArrow)) MoveLeft(); //it'll always go right if both keys are pressed, unless you can only go right, while both are pressed, when the left one isnt added in the meantime
